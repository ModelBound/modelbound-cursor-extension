...
import { reportOutcome, showReliability, type Verdict } from "./outcomes.js";
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      async (arg?: { slug?: string; verdict?: Verdict } | vscode.Uri) => {
        const slug =
          arg && typeof arg === "object" && "slug" in arg && arg.slug
            ? arg.slug
            : slugFromUri(activeSkillUri(arg as vscode.Uri | undefined)!);
        const verdict = arg && typeof arg === "object" && "verdict" in arg ? arg.verdict : undefined;
        await reportOutcome(getApi, slug, verdict);
      },
    ),

    vscode.commands.registerCommand("modelbound.reliability", async () => {
      await showReliability(getApi);
    }),
